...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        if self.use_onnx:
            ...
            encoder_out = encoder_outs[0]["encoder_out"][0].cpu().numpy()
            encoder_padding_mask = encoder_outs[0]["encoder_padding_mask"][0].cpu().numpy() if len(encoder_outs[0]["encoder_padding_mask"]) > 0 else None
            ...
            probs = self.single_model.get_normalized_probs((logits, None), log_probs=True, sample=None)
            probs = probs[:, -1, :]
