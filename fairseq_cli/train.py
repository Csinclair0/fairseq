...
def main(cfg: FairseqConfig) -> None:
    ...
    if cfg.task._name == "multilingual_denoising_universe":
        ...
        if cfg.checkpoint.finetune_from_model is not None:
            if cfg.distributed_training.distributed_rank == 0: torch.save(pt_checkpoint, cfg.checkpoint.finetune_from_model[:-3]+"_extended.pt")
            cfg.checkpoint.finetune_from_model = cfg.checkpoint.finetune_from_model[:-3]+"_extended.pt"
        else:
            if cfg.distributed_training.distributed_rank == 0: torch.save(pt_checkpoint, cfg.checkpoint.restore_file[:-3]+"_extended.pt")
